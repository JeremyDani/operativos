...
class ParticipacionAdmin(SimpleHistoryAdmin):
    list_display = ('origen', 'cedula', 'operativo', 'fecha_registro')
    search_fields = ('cedula', 'operativo__titulo')
    list_filter = ('operativo', 'origen')
    # Campos que se mostrarán en el formulario para añadir una nueva participación
    add_fields = ('origen', 'cedula', 'operativo')

    def get_fields(self, request, obj=None):
        if obj:
            # Campos a mostrar en la vista de edición (ninguno editable)
            return ('origen', 'cedula', 'operativo', 'fecha_registro')
